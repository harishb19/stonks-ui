...
const CurrencyConverter = ({image, symbol, value}) => {
    ...
    const [isUsdToCoin, setIsUsdToCoin] = useState(false);
    ...
    const switchCoin = () => {
        if (isUsdToCoin) {
            setUsdText(coinText * value)
        } else {
            setUsdText(parseFloat((coinText / value).toFixed(7)))
        }
        setIsUsdToCoin(!isUsdToCoin)
    }
    ...
    return <Box className={coinStyle.frostedDiv}>
        ...
        <Stack direction={"row"}
            ...
            <Stack direction={"row"} spacing={1} sx={{backgroundColor: "#181818", padding: '10px 20px'}}>
                <img src={isUsdToCoin ? image : '/dollar-symbol.png'} alt={isUsdToCoin ? symbol : "USD"}
                     className={currencyStyle.coinImage} onError={onMediaFallback}/>
                <Typography htmlFor={"currInput2"} variant={'body1'} component={'label'}>
                    {isUsdToCoin ? symbol.toUpperCase() : "USD"}
                </Typography>
            </Stack>
            <div style={{borderLeft: "1px solid #222324"}}/>
            <InputBase
